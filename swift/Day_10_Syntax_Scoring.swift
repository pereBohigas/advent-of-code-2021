  ...
  for character in line {
    if chunkCharacters.map(\.opening).contains(character) {
      ...
      if chunkStart == charactersStack.last! {
        ...
        let expectedClosing = chunkCharacters.filter { $0.opening == charactersStack.last! }.first!.closing
        print("\(line) - Expected \(expectedClosing), but found \(character) instead.")
