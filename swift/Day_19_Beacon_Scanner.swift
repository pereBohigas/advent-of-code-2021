...
typealias ScannerReport = (scanner: Int, beacons: [Coordinates])

let scannerHeaderPattern = #"(?<=--- scanner )[0-9]+(?= ---)"#

// let regex = try! NSRegularExpression(pattern: pattern, options: [])

let scannerReports: [ScannerReport] = input.components(separatedBy: "\n\n")
  .filter { !$0.isEmpty }
  .map {
    let data = $0.split(separator: "\n")

    let scannerNumber = data.compactMap {
        guard let range = $0.range(of: scannerHeaderPattern, options: .regularExpression) else {
          return nil
        }

        return String($0[range])
      }
      .compactMap { Int($0) }
      .first!

    let beacons: [Coordinates] = data.filter {
        $0.range(of: scannerHeaderPattern, options: .regularExpression) == nil
      }
      .map {
        $0.split(separator: ",")
          .compactMap { Int($0) }
    ...
    return (scannerNumber, beacons)
  }

print(scannerReports)
